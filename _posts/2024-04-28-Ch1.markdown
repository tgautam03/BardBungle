...
Whenever someone talks about Transformers now a days, more often than not, they mean Generative models like GPT. However, not a lot of people know that originally Transformers were designed for Machine Translation. A user would input a sentence in one language and the model's task was to translate that into a different language. This was done using an encoder-decoder styled architecture, where the encoder accepts the input and understands the context in this particular sentence. The encoded information is then passed over to the decoder which starts generating the translation. 

In this work, I'll be focusing on the task of Machine Translation using the "Shakespeare" dataset (where the model will translate shakespeare senteces to simple/modern english). Starting with:

- The dataset (which contains the excerpts from Shakespeare and their meaning), I will discuss how it is tokenized and preprocessed for training the Neural Net.

- I will then spend some time talking out token embeddings and positional encodings. The special thing about these embeddings is that they will be trained (alongside the network parameters).
...
- I will finally discuss the training procedure in detail and more specifically how the loss function enables the generator to make accurate predictions.
...
In Figure 1, we can see that most of the sentences are under 100 tokens long and the maximum sequence length is just over 200.So to cover all the possible sequence sizes, I should go with the max sequence length of 256. However, to keep things simple for these blog posts, I will filter out the sequences with lengths upto 10, and set the maximum sequence length to 15.
...
```
```
Src Shapes: torch.Size([8809, 15]); dtype: torch.int64
Input Trg Shapes: torch.Size([8809, 15]); dtype: torch.int64
Output Trg Shapes: torch.Size([8809, 15]); dtype: torch.int64
```
...
Passing the tokenized (and numericalized) dataset through these layers transform each token into a vector of length `emb` (I have set that as 32). Figure 2 and 3 shows the embedding matrix for one of the shakespeare and modern english sentences respectively.
...
Numericalized Sentence:  tensor([ 107,   48,   23,  427,   12,   87,   50,    1530,    6,   26,    2,    0,    0,    0,    0])

Token Embedded Sentence: 
```
<div class="imgcap">
<img src="https://raw.githubusercontent.com/tgautam03/BardBungle/gh-pages/assets/ch1/src_tok_emb.png" alt="this slowpoke moves"  width="800"/>
...
Tokenized Sentence:  ['Every', 'man', 'you', 'meet', 'these', 'days', 'is', 'frown', 'ing', '.', '[EOS]', '[PAD]', '[PAD]', '[PAD]', '[PAD]']

Numericalized Sentence:  tensor([   1,    1953,   66,   10,    506,    161,    842,   20,    1534,   38,    5,    0,    0,    0,    0])

Token Embedded Sentence: 
```
<div class="imgcap">
<img src="https://raw.githubusercontent.com/tgautam03/BardBungle/gh-pages/assets/ch1/trg_tok_emb.png" alt="this slowpoke moves"  width="800"/>
...
Tokenized Sentence:  ['The', 'boy', 'thank', 'ed', 'the', 'king', '[EOS]', '[PAD]', '[PAD]', '[PAD]', '[PAD]', '[PAD]', '[PAD]', '[PAD]', '[PAD]']

Numericalized Sentence:  [83, 360, 496, 39, 9, 118, 2, 0, 0, 0, 0, 0, 0, 0, 0]

Embedding Vector for word boy:  [ 0.581  0.929  0.898  0.636  0.009 -0.105 -0.602 -0.772 -0.158 -1.653 1.325 -0.95  -0.454 -0.35  -0.323  0.129 -0.83   0.129 -0.233 -0.872 2.567 -0.949  0.38   1.328  0.633 -1.318  1.373 -0.51  -0.683  0.357 1.145 -0.294]

-------------------Second Sentence-------------------------
Original Sentence:  The king thanked the boy

Tokenized Sentence:  ['The', 'king', 'thank', 'ed', 'the', 'boy', '[EOS]', '[PAD]', '[PAD]', '[PAD]', '[PAD]', '[PAD]', '[PAD]', '[PAD]', '[PAD]']

Numericalized Sentence:  [83, 118, 496, 39, 9, 360, 2, 0, 0, 0, 0, 0, 0, 0, 0]

Embedding Vector for word boy:  [ 0.581  0.929  0.898  0.636  0.009 -0.105 -0.602 -0.772 -0.158 -1.653 1.325 -0.95  -0.454 -0.35  -0.323  0.129 -0.83   0.129 -0.233 -0.872 2.567 -0.949  0.38   1.328  0.633 -1.318  1.373 -0.51  -0.683  0.357 1.145 -0.294]
```

In the original "Attention is all you need" paper, to add positional information to the embeddings, they used static vectors, but I'm again going to use the `Embedding` layer. The difference here (compared to the token embedding layer) is that I'm now mapping each position in the sequence to a vector of length equal to the embedding dimension. In simple words, as I have the max sequence length of 15, the positional embedding layer creates a unique vector for each of the 15 positions in the sequence. 

```py
# Positional Encodings
src_pos_enc = torch.nn.Embedding(num_embeddings=max_seq_len, embedding_dim=emb)
trg_pos_enc = torch.nn.Embedding(num_embeddings=max_seq_len, embedding_dim=emb)
```
...
```
```
Embedding Vector with positional encoding for word boy:  [ 0.083  1.29   1.214  1.888 -0.518 -0.308 -0.069  0.19   0.852 -0.746 1.352 -1.27   0.23   0.627  0.251 -0.27  -0.362  0.625 -0.111 -0.263 2.799 -1.811 -0.425  2.932 -0.232 -2.046  1.585  0.733  0.137  1.679 0.814  0.997]

Embedding Vector with positional encoding for word boy:  [-0.566 -0.801 -1.09   1.753  0.362  1.34   0.261 -1.005 -1.372 -0.849 3.423 -1.563 -0.47   0.67  -0.217 -0.759 -1.767  0.547 -0.552 -1.349 3.889  1.574  0.884  1.66   0.118 -0.676 -0.133  0.469  0.651  0.278 0.018 -1.29 ]
```
